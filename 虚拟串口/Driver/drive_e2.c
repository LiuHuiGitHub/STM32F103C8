 ...
 * 调用  ：外部调用
 */
void dr_e2WriteBuffer(UINT8 *pBuffer, UINT8 WriteAddr, u16 NumByteToWrite)
{
    ...
    if(Addr == 0)
    {
        ...
        if(NumOfPage == 0)
        {
            dr_e2WritePage(pBuffer, WriteAddr, NumOfSingle);
            dr_e2WaitStandby();
        }
        /* If NumByteToWrite > I2C_PageSize */
        else
        {
            while(NumOfPage--)
            {
                dr_e2WritePage(pBuffer, WriteAddr, I2C_PageSize);
                dr_e2WaitStandby();
    ...
    else
    {
        ...
        else
        {
            ...
            if(count != 0)
            {
                dr_e2WritePage(pBuffer, WriteAddr, count);
                dr_e2WaitStandby();
                WriteAddr += count;
                pBuffer += count;
            }
